...
public:
    ...
    bool operator()(const T* const pose, T* residuals) const {
        ...
        // Cast away const-ness for PoseIncrement (safe since we only read from pose)
        PoseIncrement<T> pose_inc(const_cast<T*>(pose));
        
        // Convert source and target points to templated type
        T src[3] = { T(m_sourcePoint[0]), T(m_sourcePoint[1]), T(m_sourcePoint[2]) };
        T tgt[3] = { T(m_targetPoint[0]), T(m_targetPoint[1]), T(m_targetPoint[2]) };
        
        // Transform source point
        T src_trans[3];
        pose_inc.apply(src, src_trans);
        
        // Compute residuals with templated weight
        T weight = T(m_weight);
        residuals[0] = weight * (src_trans[0] - tgt[0]) * T(LAMBDA);
        residuals[1] = weight * (src_trans[1] - tgt[1]) * T(LAMBDA);
        residuals[2] = weight * (src_trans[2] - tgt[2]) * T(LAMBDA);
        return true;
    }
...
public:
    ...
    bool operator()(const T* const pose, T* residuals) const {
        ...
        // Convert points and normal to templated type
        T src[3] = { T(m_sourcePoint[0]), T(m_sourcePoint[1]), T(m_sourcePoint[2]) };
        T tgt[3] = { T(m_targetPoint[0]), T(m_targetPoint[1]), T(m_targetPoint[2]) };
        ...
        // Compute point-to-plane residual: n · (transformed_source - target)
        T diff[3] = { src_trans[0] - tgt[0], src_trans[1] - tgt[1], src_trans[2] - tgt[2] };
        T residual = normal[0] * diff[0] + normal[1] * diff[1] + normal[2] * diff[2];
        
        // Apply weight
        T weight = T(m_weight);
        residuals[0] = T(LAMBDA) * weight * residual;
    ...
    void pruneCorrespondences(const std::vector<Vector3f>& sourceNormals, const std::vector<Vector3f>& targetNormals, std::vector<Match>& matches) {
        ...
        for (unsigned i = 0; i < nPoints; i++) {
            ...
            if (match.idx >= 0) {
                ...
                // if (angle > 60.0) {
                float dot_product = sourceNormal.dot(targetNormal);
                if (dot_product < 0.5f) {  // cos(60°) = 0.5
                    match.idx = -1;
                    continue;
    ...
    void prepareConstraints(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals, const std::vector<Match> matches, const PoseIncrement<double>& poseIncrement, ceres::Problem& problem) const {
        ...
        for (unsigned i = 0; i < nPoints; ++i) {
            ...
            if (match.idx >= 0) {
                ...
                ceres::CostFunction* cost_function = PointToPointConstraint::create(sourcePoint, targetPoint, 1.0f);
                ...
                if (m_bUsePointToPlaneConstraints) {
                    ...
                    // to the Ceres problem.
                    ceres::CostFunction* cost_function = PointToPlaneConstraint::create(sourcePoint, targetPoint, targetNormal, 1.0f);
                    problem.AddResidualBlock(cost_function, nullptr, poseIncrement.getData());
...
private:
    ...
    Matrix4f estimatePosePointToPlane(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals) {
        ...
        for (unsigned i = 0; i < nPoints; i++) {
            ...
            // Point-to-plane constraint: n · (Rs + t - d) = 0
            // Linearized form: (-ny*sz + nz*sy)*α + (nx*sz - nz*sx)*β + (-nx*sy + ny*sx)*γ + nx*tx + ny*ty + nz*tz = nx*dx + ny*dy + nz*dz - nx*sx - ny*sy - nz*sz
            A(4*i, 0) = -n.y()*s.z() + n.z()*s.y();  // coefficient for α
            A(4*i, 1) = n.x()*s.z() - n.z()*s.x();   // coefficient for β  
            A(4*i, 2) = -n.x()*s.y() + n.y()*s.x();  // coefficient for γ
            A(4*i, 3) = n.x();                        // coefficient for tx
            A(4*i, 4) = n.y();                        // coefficient for ty
            A(4*i, 5) = n.z();                        // coefficient for tz
            b(4*i) = n.x()*d.x() + n.y()*d.y() + n.z()*d.z() - n.x()*s.x() - n.y()*s.y() - n.z()*s.z();

            // TODO: Add the point-to-point constraints to the system
            // Point-to-point constraints: Rs + t = d
            // Three constraints (one for each coordinate):
            
            // x-coordinate: sx - γ*sy + β*sz + tx = dx
            // Rearranged: 0*α + sz*β + (-sy)*γ + 1*tx + 0*ty + 0*tz = dx - sx
            A(4*i+1, 0) = 0;      // coefficient for α
            A(4*i+1, 1) = s.z();  // coefficient for β
            A(4*i+1, 2) = -s.y(); // coefficient for γ
            A(4*i+1, 3) = 1;      // coefficient for tx
            A(4*i+1, 4) = 0;      // coefficient for ty
            A(4*i+1, 5) = 0;      // coefficient for tz
            b(4*i+1) = d.x() - s.x();

            // y-coordinate: γ*sx + sy - α*sz + ty = dy
            // Rearranged: (-sz)*α + 0*β + sx*γ + 0*tx + 1*ty + 0*tz = dy - sy
            A(4*i+2, 0) = -s.z(); // coefficient for α
            A(4*i+2, 1) = 0;      // coefficient for β
            A(4*i+2, 2) = s.x();  // coefficient for γ
            A(4*i+2, 3) = 0;      // coefficient for tx
            A(4*i+2, 4) = 1;      // coefficient for ty
            A(4*i+2, 5) = 0;      // coefficient for tz
            b(4*i+2) = d.y() - s.y();

            // z-coordinate: -β*sx + α*sy + sz + tz = dz
            // Rearranged: sy*α + (-sx)*β + 0*γ + 0*tx + 0*ty + 1*tz = dz - sz
            A(4*i+3, 0) = s.y();  // coefficient for α
            A(4*i+3, 1) = -s.x(); // coefficient for β
            A(4*i+3, 2) = 0;      // coefficient for γ
            A(4*i+3, 3) = 0;      // coefficient for tx
            A(4*i+3, 4) = 0;      // coefficient for ty
            A(4*i+3, 5) = 1;      // coefficient for tz
            b(4*i+3) = d.z() - s.z();

            //TODO: Optionally, apply a higher weight to point-to-plane correspondences
            // Apply higher weight to point-to-plane constraint
            float weight_plane = 1.5f;  // Can be adjusted to give more importance to point-to-plane constraints
            A.row(4*i) *= weight_plane;
            b(4*i) *= weight_plane;
